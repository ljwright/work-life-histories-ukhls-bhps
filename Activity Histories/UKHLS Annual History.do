...
*ii. Drop participants who do not have annual employment histories in a given wave.
	* Create flags for how individuals are routed through annual history modules.
	/// Participants asked both notempchk and empchk (Route3) pose a problem which I haven't yet solved as it is not clear what the wordings are of the questions asked to these participants.
		/// Further, issues with Route3 participants differ based on the answers to notempchk, empchk and subsequent questions.
		/// Some routes are more tractable than others.
qui{
...
2A. Harmonise and save spell end reason data.
	/// Recording of end reason data changed in Wave XX. 
		/// Previously, main reason for job end recorded only in a single variable. From Wave XX onwards, each reason recorded in separate variables.
	/// Note, nxtendreas missing from Annual History routing up to Wave XX (I think by mistake).
	/// End reason data kept separate to main dataset as contains 1+(3+Max_Spells)*12 extra variables
*/
...
// 	replace Source_Variable1=Source_Variable1+"; jbsamr_w"+strofreal(Wave) /*
// 		*/ if inrange(Status1,1,2) & jbsamr==2
// 	replace Source_Variable1=Source_Variable1+"; samejob_w"+strofreal(Wave) /*
// 		*/ if inrange(Status1,1,2) & samejob==2	
