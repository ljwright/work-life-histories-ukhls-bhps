...
// foreach i of numlist 1 2 5 6{
// 	prog_ftetables _`i'
// 	}
foreach var of varlist LH_*IN_MY S_*IN_MY F_*IN_MY{
	if substr("`var'",1,1)=="S" replace	`var'=. if `var'_Age>19
	if substr("`var'",1,1)=="F" replace `var'=. if `var'_Age>23
	if substr("`var'",1,1)=="L" replace `var'=. if `var'_Age>26
	local j=subinstr("`var'","_MY","",.)
	prog_scrubvars `j'
	}
drop *_Age
*	CAN DO BETTER THAN THIS USING QUALIFICATIONS.

//	NOW DECIDED TO NOT USE PROXY FOR ANY EVENT IN PARTICIPANTS PAST.
...
//	PREFER FULL INTERVIEW RESPONSES TO PROXY
	//	NOT SURE IF SENSIBLE AS GIVES GOOD INFORMATION ON FIN DATE AFTER IN.
	// 	MIGHT BE POOR FOR _NO_ THOUGH. (SORTED NOW?)
foreach i in LH S F{
...
/* DERIVE FTE FROM S OR F */
	// NEED TO USE QUALIFICATIONS BETTER FOR ESTIMATING REASONABLE F_IN F_FIN, ETC.
...
// NEED TO MAKE SURE I DON'T CREATE ANY SILLY FTE_IN DATES.

// TOMORROW:
//	PULL THROUGH QUALIFICATIONS VARIABLES. USE THIS TO CHECK FINISH DATE IS REASONABLE.
//  AMEND ACTIVITY HISTORIES TO INCORPORATE THIS FIRST LEAVING FTE INFORMATION.
// 	CREATE GLOBALS FOR LONG MISSING IFS (E.G. ALL F_*MY ARE MISSING; ALL S_*MY ARE MISSING.
